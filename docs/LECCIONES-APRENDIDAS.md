...
4. **Sandbox `scripts/opencode-sandbox.sh`** (bubblewrap): escrito y con sintaxis validada, pero **NO probado**: este sistema tiene `kernel.apparmor_restrict_unprivileged_userns=1` y sin perfil AppArmor bwrap → `bwrap: setting up uid map: Permission denied`. Habilitarlo requiere modificar el sistema (sysctl/AppArmor con sudo) → pendiente de decisión del programador (P0.5/P1.8), nunca acción por cuenta propia.
...
**Estado**: investigación y red-team cerradas; sandbox pendiente de decisión del programador sobre el sistema (restricción AppArmor/userns).
