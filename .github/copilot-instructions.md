...
6. No inventes APIs, resultados, archivos, configuraciones ni secretos.
7. No ejecutes acciones destructivas, sobre producción o sobre credenciales.
8. Reporta evidencia real, fallos y elementos no verificados; no ocultes errores.
...
Para validar requisitos usa `scripts/doc_validator.py` con Python 3. En Windows
puedes ejecutar `scripts/verificar-requisitos.ps1` o
`scripts/verificar-requisitos.cmd`. No asumas que un permiso de Copilot equivale
a un bloqueo determinista: los bloqueos dependen del runtime o sandbox activo.
