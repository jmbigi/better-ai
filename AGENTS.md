...
- **PROHIBIDO SIN EXCEPCIONES: borrado completo e irreversible de base de datos de producción** (`DROP DATABASE`, `DROP TABLE` sin filtro, `TRUNCATE` sin `WHERE`, `DELETE` sin `WHERE`, `pg_dump --clean` + restore destructivo, `mysqldump` + `DROP` + restore). Esto incluye operaciones indirectas a través de scripts de migración, CI/CD, orquestadores, cron jobs, o restauración de backups sin verificación previa.
- **PROHIBIDO SIN EXCEPCIONES: borrado completo e irreversible de archivos de configuración y entorno de producción** (`.env`, `config.yaml`, `application.properties`, `nginx.conf`, `docker-compose.yml`, `kubernetes manifests`, `terraform state`, `ansible vaults`, `/etc/` configs). Cualquier sobrescritura, eliminación o truncado de estos archivos en producción requiere 3 confirmaciones + frase exacta "Confirmo borrado configuracion produccion".
...
- **PROHIBIDO actualizar el sistema operativo del servidor de producción o aplicar parches sin plan de recuperación de desastres verificado**. Cualquier actualización de OS en entorno productivo requiere plan de rollback probado, ventana de mantenimiento aprobada y backup verificado. La recuperación de desastres no es una opción "después del hecho": debe estar preparada y testeada ANTES.
