...
| `opencode.json` | **Guardarraíles deterministas** para opencode: 304 patrones bash (218 `deny`, 85 `ask`, 1 `allow` por defecto) que bloquean comandos destructivos, acceso a `.env`/`~/.ssh`/`~/.aws`/claves (`id_rsa`, `*.pem`, `*credentials*`) por comandos comunes (`cat`/`less`/`head`/`tail`/`grep`/redirecciones) y ediciones de `.env`; incluye variantes con rutas absolutas (`*/rm -rf *`, `*/git reset --hard*`, `*/sqlite3 *DROP*`, etc.) y envoltorias `sh -c`/`bash -c` para cerrar evasiones directas detectadas por `scripts/fuzz-denies.py`; `read`/`edit` deniegan también rutas de claves y credenciales; `experimental.policies` (deny all + allow `opencode`, `opencode-go`, `kilo`, `deepseek`, `ollama`) solo permite los proveedores de modelos autorizados (decisión de coste; `ollama` es exclusivamente para la matriz de pruebas P0/P1, nunca modelo principal) y `agent` define perfiles deterministas de muestreo (`temperature`/`top_p`). A diferencia de las reglas de texto, un `deny` no se puede ignorar. |
| `kilo.json` | **Guardarraíles deterministas** para kilocode: 304 patrones bash (218 `deny`, 85 `ask`, 1 `allow` por defecto) idénticos a `opencode.json` pero con `experimental.policies` (deny all + allow `kilo`, `deepseek`, `openrouter`, `ollama`; `ollama` exclusivamente para matriz de pruebas P0/P1). Los archivos config son equivalentes por herramienta; copia el que corresponda al agente que uses. |
...
| `scripts/probar-denies.sh` | **Red-team de los guardarraíles**: prueba 154 variantes canónicas seguras de los `deny` de `opencode.json` contra el matcher REAL de opencode (config mínima aislada, sin AGENTS.md) y falla si alguna no bloquea. Las 218 reglas `deny` son idénticas en `kilo.json` (kilocode); 154 variantes fueron verificadas (15-08-2026). Uso: `bash scripts/probar-denies.sh` |
