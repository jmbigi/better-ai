...
| `opencode.json` | **Guardarraíles deterministas** para opencode: 245 patrones bash (158 `deny`, 86 `ask`, 1 `allow` por defecto) que bloquean comandos destructivos, acceso a `.env`/`~/.ssh`/`~/.aws`/claves (`id_rsa`, `*.pem`, `*credentials*`) por comandos comunes (`cat`/`less`/`head`/`tail`/`grep`/redirecciones) y ediciones de `.env`; `read`/`edit` deniegan también rutas de claves y credenciales; `enabled_providers: ["opencode", "opencode-go"]` solo carga los proveedores de modelos permitidos (decisión de coste). A diferencia de las reglas de texto, un `deny` no se puede ignorar. |
| `kilo.json` | **Guardarraíles deterministas** para kilocode: 245 patrones bash (158 `deny`, 86 `ask`, 1 `allow` por defecto) idénticos a `opencode.json` pero con `enabled_providers: ["kilo", "deepseek", "openrouter"]`. Los archivos config son equivalentes por herramienta; copia el que corresponda al agente que uses. |
