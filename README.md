...
| `scripts/opencode-sandbox.sh` | **Sandbox opcional con bubblewrap**: ejecuta opencode con toda la máquina en solo lectura salvo el workspace y las rutas de opencode (red bloqueada salvo `--net`). La capa determinista de sistema operativo por encima de los deny. Requiere `bwrap` y user namespaces habilitados en el sistema. Uso: `bash scripts/opencode-sandbox.sh [--net] [comando...]` |
...
## Los 35 errores de LLM que se previenen
